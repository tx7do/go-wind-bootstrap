...
func resolveBroker(ctx context.Context, cfg *v1.Broker) (func(), error) {
	type field struct {
		name    string
	...
	if len(fields) == 0 {
		return nil, fmt.Errorf("bootstrap: no broker specified")
	}

	var cleanups []func()
	for _, f := range fields {
		cleanup, err := f.builder(ctx, cfg)
		if err != nil {
			for _, c := range cleanups {
				c()
			}
			return nil, fmt.Errorf("bootstrap: build broker %q: %w", f.name, err)
		}
		if cleanup != nil {
